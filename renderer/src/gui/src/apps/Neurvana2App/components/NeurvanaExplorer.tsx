...
import * as neurvanaService from '../services/neurvanaService';
...
export interface NeurvanaExplorerProps {
...
export interface NeurvanaExplorerMethods {
...
const NeurvanaExplorer: React.FC<NeurvanaExplorerProps> = ({ 
  projectId, 
  onEntitySelect,
  ...
  useEffect(() => {
    ...
    if (projectId) {
      ...
      neurvanaService.saveProjectIdToUrlAndStorage(projectId);
      return;
    }
    
    // Otherwise check URL/localStorage
    const savedId = neurvanaService.getProjectIdFromUrlOrStorage();
    if (savedId) {
  ...
  useEffect(() => {
    if (localProjectId) {
      neurvanaService.saveProjectIdToUrlAndStorage(localProjectId);
    }
  }, [localProjectId]);
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        // Use the neurvanaService instead of fileService
        const projects = await neurvanaService.getProjects();
  ...
  const fetchFileTree = useCallback(async () => {
    if (localProjectId) {
      try {
        ...
        try {
          const project = await neurvanaService.getProject(localProjectId);
          if (project) {
        ...
        const entities = await neurvanaService.getProjectEntities(localProjectId);
  ...
  const handleDrop = useCallback(
    async (acceptedFiles: File[]) => {
      if (!localProjectId) {
        ...
        try {
          const newProject = await neurvanaService.createProject('New Project');
        ...
        for (const [dirPath, files] of filesByDirectory.entries()) {
          ...
          const uploadPromises = files.map(async (file) => {
            try {
              ...
              const entity = await neurvanaService.addFileToProject(file, projectId);
          ...
          if (dirPath !== 'root' && uploadedEntities.length > 0) {
            try {
              ...
              const group = await neurvanaService.createGroup(
                folderName, 
                projectId,
                childEntityIds
              );
    ...
    if (newGroupName.trim()) {
      try {
        ...
        if (!localProjectId) {
          // If no project is open, create a new one
          try {
            const newProject = await neurvanaService.createProject(newGroupName);
          ...
          try {
            ...
            const group = await neurvanaService.createGroup(newGroupName, localProjectId);
  ...
  const handleNewFileNameSubmit = async (e: React.FormEvent) => {
    ...
    if (newFileName.trim() && localProjectId) {
      try {
        ...
        const fileEntity = await neurvanaService.addFileToProject(file, localProjectId);
  ...
  const handleNewUIViewNameSubmit = async (e: React.FormEvent) => {
    ...
    if (newUIViewName.trim() && localProjectId) {
      try {
        ...
        const uiView = await neurvanaService.createUIView(
          newUIViewName,
  ...
  const addHardcodedUINodes = async (nodes: FileNode[]): Promise<FileNode[]> => {
    ...
    if (!hasIngestionWizard && localProjectId) {
      try {
        ...
        const ingestionWizard = await neurvanaService.createUIView(
          'Ingestion Wizard',
    ...
    if (!hasEntityTable && localProjectId) {
      try {
        ...
        const entityTable = await neurvanaService.createUIView(
          'Entity Table',
  ...
  const handleEntitySelect = (nodeId: string, entityType: string, url?: string) => {
    ...
    if (entityType === 'UIView' && url) {
      ...
      // Let NeurvanaApp handle the details of creating the UIViewInfo
  ...
  const handleDeleteEntity = async (node: FileNode) => {
    ...
    try {
      await neurvanaService.deleteEntity(node.id);
  ...
  const explorerMethods: NeurvanaExplorerMethods = {
  ...
  useEffect(() => {
    if (window) {
      (window as any).neurvanaExplorerMethods = explorerMethods;
    }
  }, []);
...
export default NeurvanaExplorer; 